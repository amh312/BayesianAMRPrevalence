...
###Bind results into summary dataframe
...
valid_norms_plot <- ggplot(valid_norms, aes(x=Antimicrobial,y=ratediff,fill=Antimicrobial))+
  ...
  #set colour palette with vector (see above)
  scale_fill_manual(values=col_vector) +
  
  #themes
  theme(legend.position = "none") +
  
  #central line of no difference
  geom_hline(yintercept=0,linetype=2) +
  
  #extreme prevalence estimation error lines
...
valid_plot <- ggplot(valid, aes(x=Antimicrobial,y=ratediff,fill=Antimicrobial))+
  ...
  #set colours using palette vector (see above)
  scale_fill_manual(values=col_vector) +
  theme(legend.position = "none") +
  
  #central and extreme estimation error lines
  geom_hline(yintercept=0,linetype=2) +
  geom_hline(yintercept=-10,linetype=3,colour="red") +
  geom_hline(yintercept=10,linetype=3,colour="red")
print(valid_plot)

##Resource efficiency Cleveland dotplot

###Set BEAR and OP labels on data and label plot df
...
###Calculate differences between BEAR and OP
...
resource_plot <- ggplot(numbs, aes(x=Antimicrobial,y=n)) +
  ...
  #line denoting gap between approaches
  geom_line(aes(group=Antimicrobial),alpha=0.5)+
  
  #points denoting performance of bear and op approaches
  ...
  #blue for bear, green for op
  scale_color_manual(values=c("blue","green3"))+
  
  #annotate with performance differences
